# -*- coding: utf-8 -*-
"""
Created on Tue May 23 17:04:29 2023
...
file= open("../AES_HOST_24_LFSR_seed_list_1000_repeat_20k_ct_all.csv", "r")

HD_file= "AES_HOST_24_LFSR_seed_list_1000_repeat_20k_ct_all_19th_ct_xor_textbytes.npy"
bits=128
scale = 16
...
#inv_shift_pattern = [0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3]
shift_pattern = [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11]
key_list=[44,18,44,62,215,249,69,103,35,186,126,14,169,235,80,204]

def lookup(byte):
    x = byte >> 4
    y = byte & 15
    #print("row",hex(x),"column",hex(y))
    return aes_sbox_inv[x][y]


textbytes=np.zeros((len(lines), 16),dtype=int)
for l in trange (0, len(lines)):
    memory_str=lines[l]
    
    memory_all=bin(int(memory_str, scale))[2:].zfill(bits)
    
    #print(memory_all)
    
    for n in range (0, 16):        
        memory=memory_all[8*n:(n+1)*8]
        #print(memory)
        
        init_memory_index=shift_pattern[n]
        #print(init_memory_index)
        init_memory=memory_all[8*init_memory_index:(init_memory_index+1)*8]
             
        key_xor=int(memory, 2)^ key_list[n]
        
        sboxed=bin(lookup(key_xor))[2:].zfill(8)
        
        #print(sboxed)
        
        hd_memory=bin(int(init_memory, 2) ^ int(sboxed, 2))[2:].zfill(8)
        textbytes[l, n]=int(hd_memory,2)
        print(textbytes[l, n])

        
np.save(HD_file,textbytes)
