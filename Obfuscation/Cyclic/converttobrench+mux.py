'''
Python code for coverting to bench file
...
def gate2X1(x):
    ...
    input_node2 =input_node2.strip(')')

    if (gate_type_name== "MX2X1"):        
            
        input_node3 =gate_nodes[3].strip('.S0(')
        if(gate_nodes[3][len(gate_nodes[3])-1]=="0"):
            input_node3=input_node3+"0"
        print("selection mux", input_node3)

        global cyc_devices_sel
        cyc_devices_sel[gate_name]=input_node3
...
def print_b_nodes():
    ...
    for dx in range(node_len):
        ...
        if d_in_len == 2:
            ...
            if(d_type=="mx"):
                d_type="mux"
                d_sel=cyc_devices_sel.get(d_name)
                new_netlist.write(d_output+" = "+d_type+"("+d_sel+", "+d_input1+", "+ d_input2+")\n")
            else:
